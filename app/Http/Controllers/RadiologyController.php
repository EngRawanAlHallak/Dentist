...
class RadiologyController extends Controller
{
    ...
    public function uploadRadiologyImage(Request $request)
    {
        ...
        foreach ($request->file('image') as $index => $image) {
            ...
            $filename = 'requestId-' . $req->id . '-patientId-' . $request->patient_id . '-' . $index . '.' . $extension;
            $path = $image->storeAs('radiology', $filename, 'public');

            $radiologyImages[] = RadiologyImage::create([
                'request_id' => $req->id,
                'image_url'  => '/storage/'.$path,
                'type'       => $request->type[$index], // match each image with its type
            ]);
        }
