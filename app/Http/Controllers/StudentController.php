...
class StudentController extends Controller
{
    ...
    public function changeDayStatus(Request $request)
    {
        ...
        if ($request->status === 'off') {
            ...
            // delete the booked appointments and notify the patient
            $appointments = Appointment::where('student_id', $student->id)
                ->where('date', $date)
                ->whereDoesntHave('session')
                ->with('request.patient.user')
                ->get();

            // Notify patients
            foreach ($appointments as $appointment) {
                $patientUser = $appointment->request->patient->user;
                if ($patientUser) {
                    $this->notificationService->notifyUser(
                        $patientUser->id ,
                        "تم الغاء موعدك لسبب طارئ",
                        "نعتذر بشدة , ان موعدك الذي تم حجزه قد الغي بسبب ظرف طارئ , تفضل بحجز موعد اخر يناسبك",
                        [
                            'student name' => $student->user->name['en'],
            ...
            //Delete these appointments
            Appointment::whereIn('id', $appointments->pluck('id'))->delete();
            ...
            return [
                ...
                'appointments' => $dailyAppointments->map(function ($appointment) {
                    ...
                    return [
                        ...
                        'patient_name' => $appointment->request->patient->user->name ?? null,//here
                        'time' => Carbon::createFromFormat('H:i:s', $appointment->time)->format('g:i A'),
    ...
    public function viewPreviousInfo (Request $request)
    {
            ...
            if ($appt->session && $appt->session->images) {
                foreach ($appt->session->images as $img) {
                    if ($img->type === 'before-treatment')
                        $beforeImages[] = '/storage/'. $img->image_url;
                    elseif ($img->type === 'after-treatment')
                        $afterImages[] = '/storage/'.  $img->image_url;
                }
            }
        }
        ...
        return response()->json([
            ...
            'x_ray' => optional('/storage/'.$x_ray)->image_url ?? null,
            'real_image' =>optional('/storage/'. $real_image)->image_url ?? null,
    ...
    public function viewPatientInfo(Request $request)
    {
        ...
        return response()->json([
            ...
            'medication' => $patient->medications->map(function ($med) {
                return asset('storage/' . $med->image_url);
            }),
        ]);
    }
    ...
    public function downloadPdf()
    {
        ...
        $filename = 'portfolio_' . Str::slug($user->name['en'] ?? 'Unknown') . '.pdf';
        ...
        return response()->json([
            ...
            'url' =>  asset('/storage/portfolios/' . $filename),
        ]);
    }
        ...
        foreach ($stages as $stage) {
            ...
            foreach ($stage->educationalContents as $content) {
                $formattedContents[] = [
                    ...
                    'images' => $content->images->map(function ($image) {
