...
class AdmissionController extends Controller
{
    ...
    public function weeklySummary()
    {
        ...
        $result = $grouped->map(function ($sessions, $stageId) {
            ...
            $assignRequests = PatientRequest::all()->where('status','processed')->count();
            $unassignRequests = PatientRequest::all()->where('status','under processing')->count();
            return [
                'stage_id' => $stageId,
                ...
                'assign_request_count' => $assignRequests,
                'unassign_request_count' => $unassignRequests,
            ];
        })->values();
    ...
    public function requestDetails(Request $request)
    {
        ...
        $radiologyImages = $req->radiologyImages->map(fn($image) => [
        ...
        return response()->json([
            ...
            'radiology_images' => '/storage/'. $radiologyImages,
        ]);
    }
    public function allStages()
    {
        $stages = PracticalSchedule::all()->map(function ($s) {
            return [
                'stage_id' => $s->stage_id,
                'name' => Stage::find($s->stage_id)->name['en'],
            ];
        })->unique('stage_id');
