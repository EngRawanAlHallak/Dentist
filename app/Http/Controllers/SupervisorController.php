...
class SupervisorController extends Controller
{
    ...
    public function storeEducationalContent(Request $request)
    {
        ...
        $content->load('images', 'stage');

        // تعديل المسارات قبل الإرجاع
        if ($content->file_path) {
            $content->file_path = '/storage/' . $content->file_path;
        }

        $content->images->transform(function ($image) {
    ...
    {
        $contents = EducationalContent::where('supervisor_id', Auth::id())
            ->with('images', 'stage')
            ->get();

        // تعديل الصور والمسارات
    ...
    private function handleSupervisorView($sessionId)
    {
        $session = Session::with([
            'images',
            'appointment.request.patient.user',
        ...
        return response()->json([
            ...
            'student_image' =>'/storage/student_profiles/'. $session->appointment->student->profile_image_url,
            'stage' => $session->appointment->request->stage->name['en'] ?? 'N/A',
            'patient_name' => $session->appointment->request->patient->user->name ?? 'N/A',
            'patient_id' => $session->appointment->request->patient->id,
            'description' => $session->description['en'],
            'radiology_image' => '/storage/'.$session->appointment->request->radiologyImages()->first()?->image_url,
            'before_images' => $session->images
                ->where('type', 'before-treatment')
                ->map(function ($image) {
                    return '/storage/' . $image->image_url;
                })
                ->values(),

            'after_images' => $session->images
                ->where('type', 'after-treatment')
                ->map(function ($image) {
                    return '/storage/' . $image->image_url;
                })
                ->values(),
    ...
    private function handleDoctorView($studentId, $sessionId)
    {
        ...
        $sessions = Session::with([
            ...
            ->whereHas('appointment.request', function ($q) use ($studentId, $stageId) {
        ...
        if ($sessions->isEmpty()) {
            return response()->json(['message' => 'No sessions found for this student in the given stage'], 404);
        }

        $data = $sessions->map(function ($session) use ($student, $stage) {
            $appointment = $session->appointment;
            $request = $appointment?->request;
            ...
            return [
                ...
                'stage_id' => $stage->id,
                'stage_name' => $stage->name['en'] ?? null,
                'student_name' => $student->user->name['en'],
                'student_image' => '/storage/student_profiles/'.$student->profile_image_url,
                'evaluation_score' => $session->evaluation_score,
                'supervisor_comments' => $session->supervisor_comments,
                'patient_name' => optional($request?->patient?->user)->name,
    ...
    public function doctorViewCase($sessionId)
    {
        ...
        return response()->json([
            ...
            'patient_id' => $session->appointment->request->patient->id,
            'description' => $session->description['en'],
            'radiology_image' => optional($session->appointment->request->radiologyImages()->first())->image_url
                ? '/storage/' . $session->appointment->request->radiologyImages()->first()->image_url
                : null,
            'before_images' => $session->images
                ->where('type', 'before-treatment')
                ->map(function ($image) {
...
///evaluate
    public function evaluateSession(Request $request)
    {
        ...
        $stage = $appointment->request->stage;
        if (!$stage) {
            return response()->json([
                'message' => 'Stage not found for this appointment request.'
            ], 404);
        }
        $sessionMaxMark = $stage->Practical_Mark / $stage->required_case_count;
        if ($request->evaluation_score > $sessionMaxMark) {
            return response()->json([
                'message' => "The maximum allowed score for this session is $sessionMaxMark.",
            ], 422);
        }
        $session->update([
        ...
        $this->notificationService->notifyUser(
            ...
            "your supervisor : $user->name evaluated your internship",
        );

        return response()->json([
            'message' => 'Session evaluated and archived status updated successfully.',
            'given_score' =>(int) $request->evaluation_score,
            'max_allowed_score' => $sessionMaxMark,
        ]);
    }
    ...
    public function getStudentsForPracticalSchedule(Request $request)
    {
        ...
        $studentsData = $schedule->students->map(function ($student) use ($schedule) {
            return [
                ...
                'profile_image_url' =>'storage/student_profiles/'. $student->profile_image_url,
                'year' => $student->year,
